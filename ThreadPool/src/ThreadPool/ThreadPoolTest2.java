...
class Counter20<V> implements Callable<V>{
	int i;
...
class CounterRun2<V> implements Runnable{
	int i;
...
class Counter10 implements Runnable{
...
public class ThreadPoolTest2 {
	...
	void TestAwaitTermination() {
		...
		tp.submit(new Counter2(i), Priority.LOW, i);
		tp.submit(new Counter2(i), Priority.LOW, i);
		tp.submit(new Counter2(i), Priority.MEDIUM, i);
	...
	void TestAwaitTerminationTime() {
		...
		tp.awaitTermination(1, TimeUnit.MICROSECONDS);
		...
		assertEquals(ans.intValue() < 8, true);
	}
